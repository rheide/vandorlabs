    ...
    public static void main(String[] args) {
        for (int design=0;design<15;design++) for (int detail=0;detail<3;detail++)
            for (boolean framed:new boolean[]{false,true}) for (int direction=0;direction<3;direction++)
                ...
                new SpaceDoorData(design,detail,framed,direction,middle).write(tag);
                SpaceDoorData copy=SpaceDoorData.read(tag);
                check(copy.design==design && copy.detail==detail && copy.framed==framed
                        && copy.direction==direction && copy.middle==middle,"settings round trip");
        ...
        check(empty.design==2 && empty.detail==1 && empty.framed && empty.direction==0 && !empty.middle,"legacy defaults");
        ...
        System.out.println("Space door settings PASS: 540 configurations, persistence, vertical travel and bounds");
    }
}
